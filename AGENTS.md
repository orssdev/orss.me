...
Window sizing is a named-preset system (`window-manager.ts`'s `WindowSizePreset`: `"small"` | `"medium"` | `"fullscreen"`), not free-form pixels. An `AppDefinition` opts into `defaultSize` (what `centeredWindow` opens it at — defaults to `"small"`) and `minSize` (the floor `FloatingWindow` resolves via `presetSize` and passes to `Rnd` as `minWidth`/`minHeight` — also defaults to `"small"`). `minSize: "fullscreen"` has no fixed pixel size to enforce as a floor, so it's implemented instead by pinning the window (`disableDragging`/`enableResizing` both off, same treatment as `isMaximized`) rather than by measuring the live screen.
